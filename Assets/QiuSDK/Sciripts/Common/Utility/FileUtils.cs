...
namespace N3DClient
{
    ...
    {
        public static void GetFileDataAsync(string path, Action<byte[]> action)
        {
            U3DTypeSDK.Instance.StartCoroutine(GetFileDataAyncIter(path, action));
        }

        private static IEnumerator GetFileDataAyncIter(string path, Action<byte[]> action)
        {
